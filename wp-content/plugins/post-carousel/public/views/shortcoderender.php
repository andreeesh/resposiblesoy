		...
		elseif ( $pc_themes == 'carousel_two' ) 
		{
			...
			if ( $que->have_posts() ) {
				while ( $que->have_posts() ) : $que->the_post();
					...
					$outline .= '<div class="date-post font-semibold py-xxs">'.get_the_date().'</div>';

					if ( $pc_post_title == 'true' ) {
						$outline .= '<h2 class="sp-pc-post-title pb-sm">';
						$outline .= '<a href="' . get_the_permalink() . '">' . get_the_title() . '</a>';
						$outline .= '</h2>';
					}

					$outline .= '<a href="'.get_the_permalink().'" class="ver-mas-post">Ver más +</a>';
