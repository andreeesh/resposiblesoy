	...
	function sc_zoom_box($attr, $content = null)
	{
		...
		$output = '<div class="zoom_box">';
			...
			if ($link) {
				...
				$output .= '<div class="desc" style="background-color:'. esc_attr(hex2rgba($bg_color, 0.8)) .';">';
...
if (! function_exists('sc_trailer_box')) {
	function sc_trailer_box($attr, $content = null)
	{
		...
		$output = '<div class="trailer_box '. esc_attr($class) .'">';
				...
				while ($query_blog->have_posts()) {
					...
					$output .= '<li class="'. esc_attr(implode(' ', get_post_class())) .'">';
						...
						$output .= '<div class="desc">';
							...
							$output .= '<div class="date">'. esc_html(get_the_date()) .'</div>';

							if ($first) {
								$output .= '<h4><a href="'. esc_url(get_permalink()) .'">'. wp_kses(get_the_title(), mfn_allowed_html()) .'</a></h4>';
							} else {
								$output .= '<h2><a href="'. esc_url(get_permalink()) .'">'. wp_kses(get_the_title(), mfn_allowed_html()) .'</a></h2>';
							}

							$output .= '<div class="desc_footer">';
								//$output .= '<span class="date">'. esc_html(get_the_date()) .'</span>';
								/*if (comments_open()) {
...
if (! function_exists('sc_divider')) {
	function sc_divider($attr, $content = null)
	{
		...
		if ($height) {
			$style_escaped = 'style="margin: 0 auto '. intval($height, 10) .'px;"';
		} else {
			$style_escaped = '';
		}
		...
		switch ($style) {
			...
			// This variable has been safely escaped above in this function
				$output = '<div class="hr_dots" '. $style_escaped .'><span></span><span></span><span></span></div>'."\n";
				break;

			case 'zigzag':

			// This variable has been safely escaped above in this function
				$output = '<div class="hr_zigzag" '. $style_escaped .'><i class="icon-down-open"></i><i class="icon-down-open"></i><i class="icon-down-open"></i></div>'."\n";
				break;
			...
			default:
			  ...
			  // This variable has been safely escaped above in this function
					$output = '<hr class="hr_narrow '. esc_attr($class) .'" '. $style_escaped .'/>'."\n";
				} elseif ($line == 'wide') {

			  // This variable has been safely escaped above in this function
					$output = '<div class="hr_wide '. esc_attr($class) .'" '. $style_escaped .'><hr /></div>'."\n";
				} elseif ($line) {

			  // This variable has been safely escaped above in this function
					$output = '<hr class="'. esc_attr($class) .'" '. $style_escaped .'/>'."\n";
				} else {

			  // This variable has been safely escaped above in this function
					$output = '<hr class="no_line" '. $style_escaped .'/>'."\n";
				}
...
if (! function_exists('sc_icon')) {
	function sc_icon($attr, $content = null)
	{
		...
		$output = '<i class="'. esc_attr($type) .'"></i>';
	...
	function sc_button($attr, $content = null)
	{
		...
		if ($align) {
			...
			$output .= '<a class="button '. esc_attr($class) .' button_js" href="'. esc_url($link) .'"'. $attributes_escaped .'>';
				...
				if ($title) {
					$output .= '<span class="button_label">'. esc_html($title) .'</span>';
				}
		...
		$output = '<div class="team team_'. esc_attr($style) .'">';
			...
			if ($animate) {
				...
				$output .= '<div class="desc_wrapper">';
					...
					if ($blockquote) {
						$output .= '<blockquote>'. esc_html($blockquote) .'</blockquote>';
					}
						...
						$output .= '<li class="offer_thumb_li id_'. esc_attr($i) .'">';
							...
							$output .= '<div class="thumbnail" style="display:none">';
								...
								if ($thumbnail = get_post_meta($id, 'mfn-post-thumbnail', true)) {
									$output .= '<img src="'. esc_url($thumbnail) .'" class="scale-with-grid" alt="" />';
...
if (! function_exists('sc_map')) {
	function sc_map($attr, $content = null)
	{
		...
		wp_enqueue_script('google-maps', 'http'. mfn_ssl() .'://maps.google.com/maps/api/js'. esc_attr($api_key), false, null, true);
		...
		$output = '<script>';
			...
			$output .= 'function google_maps_'. esc_attr($uid) .'(){';

				$output .= 'var latlng = new google.maps.LatLng('. esc_attr($lat) .','. esc_attr($lng) .');';
				...
				if ($color && ! $styles) {
					...
					if (mfn_brightness($color_escaped) == 'light') {
						// light

						$styles = '[{featureType:"all",elementType:"labels",stylers:[{visibility:"on"}]},{featureType:"administrative",elementType:"all",stylers:[{visibility:"off"}]},{featureType:"landscape",elementType:"all",stylers:[{color:"'. $color_escaped .'"},{visibility:"simplified"}]},{featureType:"poi",elementType:"all",stylers:[{visibility:"off"}]},{featureType:"road",elementType:"all",stylers:[{visibility:"on"}]},{featureType:"road",elementType:"geometry",stylers:[{color:"'. $color_escaped .'"},{lightness:"50"}]},{featureType:"road",elementType:"labels.text",stylers:[{visibility:"on"}]},{featureType:"road",elementType:"labels.text.fill",stylers:[{color:"'. $color_escaped .'"},{lightness:"-60"}]},{featureType:"road",elementType:"labels.text.stroke",stylers:[{color:"'. $color_escaped .'"},{lightness:"50"}]},{featureType:"road",elementType:"labels.icon",stylers:[{visibility:"off"}]},{featureType:"transit",elementType:"all",stylers:[{visibility:"simplified"},{color:"'. $color_escaped .'"},{lightness:"50"}]},{featureType:"transit.station",elementType:"all",stylers:[{visibility:"off"}]},{featureType:"water",elementType:"all",stylers:[{color:"'. $color_escaped .'"},{lightness:"-10"}]}]';

					} else {
						// dark

						$styles = '[{featureType:"all",elementType:"labels",stylers:[{visibility:"on"}]},{featureType:"administrative",elementType:"all",stylers:[{visibility:"off"}]},{featureType:"landscape",elementType:"all",stylers:[{color:"'. $color_escaped .'"},{visibility:"simplified"}]},{featureType:"poi",elementType:"all",stylers:[{visibility:"off"}]},{featureType:"road",elementType:"all",stylers:[{visibility:"on"}]},{featureType:"road",elementType:"geometry",stylers:[{color:"'. $color_escaped .'"},{lightness:"30"},{saturation:"-10"}]},{featureType:"road",elementType:"labels.text",stylers:[{visibility:"on"}]},{featureType:"road",elementType:"labels.text.fill",stylers:[{color:"'. $color_escaped .'"},{lightness:"80"}]},{featureType:"road",elementType:"labels.text.stroke",stylers:[{color:"'. $color_escaped .'"},{lightness:"0"}]},{featureType:"road",elementType:"labels.icon",stylers:[{visibility:"off"}]},{featureType:"transit",elementType:"all",stylers:[{visibility:"simplified"},{color:"'. $color_escaped .'"},{lightness:"50"}]},{featureType:"transit.station",elementType:"all",stylers:[{visibility:"off"}]},{featureType:"water",elementType:"all",stylers:[{color:"'. $color_escaped .'"},{lightness:"-20"}]}]';

					}
				}

				$output .= 'var myOptions = {';
					$output .= 'zoom : '. intval($zoom, 10) .',';
					$output .= 'center : latlng,';
					$output .= 'mapTypeId : google.maps.MapTypeId.'. esc_attr($type) .',';
					if ($styles) {
						$output .= 'styles : '. $styles .',';
					}
					$output .= 'draggable : draggable,';
					$output .= 'zoomControl : '. esc_attr($zoomControl) .',';
					$output .= 'mapTypeControl : '. esc_attr($mapTypeControl) .',';
					$output .= 'streetViewControl	: '. esc_attr($streetViewControl) .',';
					$output .= 'scrollwheel : false';
				$output .= '};';

				$output .= 'var map = new google.maps.Map(document.getElementById("google-map-area-'. esc_attr($uid) .'"), myOptions);';

				$output .= 'var marker = new google.maps.Marker({';
					$output .= 'position : latlng,';
					if ($icon) {
						$output .= 'icon: "'. esc_url($icon) .'",';
					}
					$output .= 'map : map';
				$output .= '});';
				...
				if ($latlng) {
					...
					foreach ($latlng as $k=>$v) {
						...
						$output .= 'var '. esc_attr($markerID) .' = new google.maps.Marker({';
							$output .= 'position : new google.maps.LatLng('. esc_attr($vEx[0]) .','. esc_attr($vEx[1]) .'),';
							if ($customIcon) {
								$output .= 'icon: "'. esc_url($customIcon) .'",';
							}
							$output .= 'map : map';
						$output .= '});';
					}
			...
			$output .= 'jQuery(document).ready(function($){';
				$output .= 'google_maps_'. esc_attr($uid) .'();';
			$output .= '});';
...
if (! function_exists('sc_progress_icons')) {
	function sc_progress_icons($attr, $content = null)
	{
		...
		$output = '<div class="progress_icons" data-active="'. esc_attr($active) .'" data-color="'. esc_attr($background) .'">';
			for ($i = 1; $i <= $count; $i++) {
				if ($image) {
					$output .= '<span class="progress_icon progress_image"><img src="'. esc_url($image) .'" alt=""/></span>';
				} else {
	...
	function sc_bar($attr, $content = null)
	{
		...
		$output  = '<li>';
			...
			$output .= '<div class="bar" '. $size_escaped .'>';
				$output .= '<span class="progress" style="width:'. intval($value, 10) .'%"></span>';
			$output .= '</div>';
	...
	function sc_timeline($attr, $content = null)
	{
		...
		$output  = '<ul class="timeline_items">';
			...
			if (is_array($tabs)) {
				...
				foreach ($tabs as $tab) {
					$output .= '<li>';
						$output .= '<h3>'. wp_kses($tab['title'], mfn_allowed_html()) .'</h3>';
						if ($tab['content']) {
							$output .= '<div class="desc">';
								$output .= do_shortcode($tab['content']);
							$output .= '</div>';
							...
							$output .= '<li>';
								...
								$output .= '<div class="single-photo-img">';
									if (has_post_thumbnail()) {
										...
										$output .= '<img class="scale-with-grid" src="'. esc_url(THEME_URI) .'/images/testimonials-placeholder.png" alt="'. esc_attr(get_post_meta(get_the_ID(), 'mfn-post-author', true)) .'" />';
									}
								$output .= '</div>';
...
/**
 * Modify default gallery shortcode
 */
